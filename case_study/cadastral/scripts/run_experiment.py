#!/usr/bin/env python3
"""
Case Study: Urban Parcel Cadastral Evolution Experiment Script
...
def run_experiment_for_model(
    ...
    start_database(uri, user, password, database)
    return {
        "model": model,
...
def main():
    ...
    parser.add_argument(
        '--queries', '-q',
        type=str,
        default='queries/cadastral_queries.json',
    ...
    # Static attribute list (cadastral case has no static attributes)
    static_attributes = []
    ...
    for model in args.models:
        try:
            ...
            print(f"\n❌ Model {model} experiment failed: {e}", file=sys.stderr)
            import traceback
            traceback.print_exc()
    ...
    print(f"\n{'=' * 60}")
    print("Experiments completed! Results saved to:", output_path)
    print(f"{'=' * 60}")

    # Print summary
    print("\nExperiment summary:")
    print(f"{'Model':<10} {'Import Time(s)':<15} {'Storage(MB)':<15} {'Nodes':<15} {'Edges':<15}")
    print("-" * 70)
    for model, result in results.items():
        if "error" in result:
            print(f"{model:<10} {'ERROR':<15}")
        else:
            print(
                f"{model:<10} {result.get('ingest_time_s', 0):<15.2f} {result.get('store_mb', 0):<15.2f} {result.get('n_nodes', 0):<15,} {result.get('n_edges', 0):<15,}")
